...
#         if not os.path.islink(os.path.join(config.public_html_folder, config.project_uuid)):
#             # The symlink has to be relative so that the web server can locate the project folder
#             relative_path = os.path.relpath(os.path.join(config.result_path, "atacseq_pipeline"))
...
#         with open(config.annotation, 'r') as sas:
...
#             report_dir = os.path.join(config.result_path, "atacseq_pipeline", 'report')
...
#                 hub_dir = os.path.abspath(os.path.join(config.result_path, "atacseq_pipeline", "hub")) # os.path.join(config.metadata['output_dir'], 'hub')
#                 if not os.path.exists(hub_dir):
#                     log.error('Please make sure that trackhub_dir exists {}'.format(hub_dir))
